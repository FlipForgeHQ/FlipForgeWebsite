(()=>{
  ...
  if(demoSection&&'IntersectionObserver' in window){
    const observer=new IntersectionObserver(entries=>{
      ...
      if(floatingDemo){
        floatingDemo.style.opacity=inView?'0':'1';
        floatingDemo.style.transform=inView?'translateY(10px)':'translateY(0)';
        floatingDemo.style.pointerEvents=inView?'none':'auto';
        floatingDemo.setAttribute('aria-hidden',inView?'true':'false');
      }
      scheduleNext();
    },{threshold:[0,.35,.7]});
    ...
    inView=true;
    if(floatingDemo){
      floatingDemo.style.opacity='0';
      floatingDemo.style.pointerEvents='none';
      floatingDemo.setAttribute('aria-hidden','true');
    }
    scheduleNext();
  }
  syncPlayback();
