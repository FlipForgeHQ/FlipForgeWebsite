(() => {
  ...
  function moneyFromCents(value) { return money(safeNumber(value) / 100); }
  ...
  function mappingConfirmed(item) { return String(item?.mappingState || "").toUpperCase() === "CONFIRMED"; }
  function mappingLabel(item) { return mappingConfirmed(item) ? "CardSight catalog linked" : "CardSight catalog link pending"; }
  ...
  function validValueIntelligence(value) {
    if (value == null) return true;
    if (typeof value !== "object" || Array.isArray(value)) return false;
    const numericFields = ["directSupportedValueCents", "referenceLowCents", "referenceMidpointCents", "referenceHighCents", "analogCompletedSaleCount"];
    return typeof value.basis === "string"
      && typeof value.confidenceBand === "string"
      && typeof value.explanation === "string"
      && typeof value.version === "string"
      && value.recommendationAuthority === false
      && value.priceAdjustmentApplied === false
      && typeof value.exactEvidenceRequired === "boolean"
      && numericFields.every(field => Number.isFinite(Number(value[field])) && Number(value[field]) >= 0)
      && safeNumber(value.referenceLowCents) <= safeNumber(value.referenceMidpointCents)
      && safeNumber(value.referenceMidpointCents) <= safeNumber(value.referenceHighCents);
  }
  ...
  function valueIntelligencePanel(item) {
    const value = item?.valueIntelligence;
    ...
    return `<section class="staging-value-intelligence" aria-label="Saved value intelligence context"><div class="staging-value-intelligence-head"><div><span class="eyebrow">Value intelligence</span><h3>Observed analog reference range</h3></div><span class="staging-value-context-badge">Context only</span></div><div class="staging-value-intelligence-grid"><div><span>Observed range</span><strong>${escapeHtml(range)}</strong></div><div><span>Reference midpoint</span><strong>${escapeHtml(moneyFromCents(value.referenceMidpointCents))}</strong></div><div><span>Analog completed sales</span><strong>${safeNumber(value.analogCompletedSaleCount)}</strong></div><div><span>Confidence band</span><strong>${escapeHtml(value.confidenceBand || "UNKNOWN")}</strong></div></div><p>${escapeHtml(value.explanation || "Governed analog completed-sale context is available for this saved card.")}</p><div class="boundary-note"><strong>Not a supported value:</strong> This observed range is saved read-only context. Exact trusted evidence is still required before it can become supported value, and it does not change the Smart Opportunity recommendation.</div></section>`;
  }
  ...
  function opportunityTable() {
    ...
    return `<div class="table-wrap"><table><thead><tr><th>Saved card</th><th>Decision</th><th>Ask</th><th>Supported</th><th>Confidence</th><th>Evidence</th><th>Catalog link</th></tr></thead><tbody>${items.map(item => { const display = cardDisplay(item); return `<tr data-customer-opportunity="${escapeHtml(item.id)}" tabindex="0"><td><strong>${escapeHtml(display.title)}</strong>${display.identity ? `<small>${escapeHtml(display.identity)}</small>` : ""}</td><td>${badge(item.recommendation || "UNKNOWN", String(item.recommendation || "unknown").toLowerCase())}</td><td>${money(item.ask)}</td><td>${escapeHtml(supportedValueText(item))}</td><td>${safeNumber(item.confidence)}/100</td><td>${safeNumber(item.evidence?.acceptedSales)}</td><td>${escapeHtml(mappingLabel(item))}</td></tr>`; }).join("")}</tbody></table></div>`;
  }
  function listView() {
  ...
  function detailView() {
    ...
    const catalogLinked = mappingConfirmed(item), evidenceReady = acceptedSales > 0, psaAvailable = psa.guidanceStatus === "SAVED_GUIDANCE_AVAILABLE";
    const catalogLinkDetail = catalogLinked
      ? "The CardSight provider catalog link is confirmed. Marketplace listing verification remains a separate server-side check."
      : "The CardSight provider catalog link is still pending. This is separate from the exact marketplace listing identity check.";
    const decisionBoundaryMessage = catalogLinked
      ? (item.statusMessage || item.authorityBoundary || "Review the saved evidence and unresolved limits before acting outside FlipForge.")
