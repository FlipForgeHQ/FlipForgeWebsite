(() => {
  ...
  function correlationId() {
    ...
    return `customer-opportunities-${Date.now()}-${Math.random().toString(16).slice(2)}`;
  }
  ...
  function allowedPath(path) {
    ...
    try {
      ...
      throw new Error("The requested saved-card identifier is invalid.");
    }
  ...
  async function parseResponse(response) {
    ...
    if (text.length > MAX_RESPONSE_CHARACTERS) {
      throw Object.assign(new Error("The Card Intelligence response exceeded the browser safety limit."), { code: "CARD_INTELLIGENCE_RESPONSE_TOO_LARGE" });
    }
    try {
      return text ? JSON.parse(text) : {};
    } catch (_) {
      throw Object.assign(new Error("The Card Intelligence gateway returned invalid JSON."), { code: "CARD_INTELLIGENCE_INVALID_JSON" });
    }
  }
  ...
  async function request(path) {
    ...
    if (!response.ok) {
      ...
      throw Object.assign(new Error(upstream.message || `Card Intelligence request failed with status ${response.status}.`), {
    ...
    if (!valid) {
      throw Object.assign(new Error("The Card Intelligence response failed the FlipForge authority contract."), { code: "CARD_INTELLIGENCE_CONTRACT_INVALID" });
    }
    return payload;
  }
  ...
  function pageHeading(title, description, actions = "") {
    return `<header class="page-heading"><div><span class="eyebrow">Tenant-owned saved intelligence</span><h1>${escapeHtml(title)}</h1><p>${escapeHtml(description)}</p></div>${actions ? `<div class="page-actions">${actions}</div>` : ""}</header>`;
  }
  ...
  function errorPanel(error) {
    ...
    const guidance = error.status === 401
      ...
      : error.status === 403
        ...
        : "No mock record or browser-generated recommendation was substituted.";
    ...
    const signIn = error.status === 401
      ? `<div class="customer-intelligence-actions"><a class="button button-primary" href="${productionHost() ? `/production-auth.html?return=${encodeURIComponent(returnPath)}` : `/staging-auth.html?returnTo=${encodeURIComponent(returnPath.replace(/^\/app/, "/saas-prototype"))}`}">Sign in securely</a></div>`
      : "";
  ...
  function metricCards() {
    ...
    const values = [
      ["Tracked decisions", safeNumber(metrics.trackedOpportunities)],
      ["Evidence ready", safeNumber(metrics.evidenceReady)],
      ["Population context", safeNumber(metrics.populationContextAvailable)],
      ["Needs verification", safeNumber(metrics.needsVerification)]
    ];
  ...
  function opportunityTable() {
    ...
    return `<div class="table-wrap"><table><thead><tr><th>Saved card</th><th>Decision</th><th>Ask</th><th>Supported</th><th>Confidence</th><th>Evidence</th><th>Mapping</th></tr></thead><tbody>${items.map(item => `<tr data-customer-opportunity="${escapeHtml(item.id)}" tabindex="0"><td><strong>${escapeHtml(item.title || item.cardIdentity || item.id)}</strong><small>${escapeHtml(item.cardIdentity || item.id)}</small></td><td>${badge(item.recommendation || "UNKNOWN", String(item.recommendation || "unknown").toLowerCase())}</td><td>${money(item.ask)}</td><td>${money(item.supportedValue)}</td><td>${safeNumber(item.confidence)}/100</td><td>${safeNumber(item.evidence?.acceptedSales)}</td><td>${escapeHtml(item.mappingState || "UNKNOWN")}</td></tr>`).join("")}</tbody></table></div>`;
  }
  ...
  function renderCurrent() {
    ...
    state.main.innerHTML = state.selectedId ? detailView() : listView();
    bindActions();
  }
  ...
  async function load() {
    ...
    try {
      ...
      if (!SAFE_ID.test(state.requestedId) || !ids.includes(state.requestedId)) {
        throw Object.assign(new Error("The requested saved card was not returned for this tenant."), { code: "RESOURCE_NOT_FOUND", status: 404 });
      }
  ...
  window.FlipForgeCustomerOpportunities = Object.freeze({
