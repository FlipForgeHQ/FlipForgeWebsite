(() => {
  ...
  function errorPanel(error) {
    ...
    const signIn = error.status === 401
      ? `<div class="customer-management-actions"><a class="button button-primary" href="${productionHost() ? `/production-auth.html?return=${encodeURIComponent(`/app/#/${state.route}`)}` : `/staging-auth.html?returnTo=${encodeURIComponent(`/saas-prototype/#/${state.route}`)}`}">Sign in securely</a></div>`
      : "";
  ...
  function offlinePanel() {
    return `<section class="panel"><div class="panel-body staging-empty"><strong>This private-beta workspace is safely offline.</strong><p>The customer gateway is disabled, so no tenant request was attempted and no sample data was substituted.</p></div></section>`;
  }
