...
function requestCandidates(urlPath) {
  ...
  if (relative === "app/customer") {
    return [join(prototypeRoot, "customer-index.html"), join(prototypeRoot, "index.html")];
  }
  if (relative.startsWith("app/customer/")) {
    const nested = relative.slice("app/customer/".length) || "customer-index.html";
    if (nested === "customer-index.html") {
      return [join(prototypeRoot, "customer-index.html"), join(prototypeRoot, "index.html")];
    }
    const candidate = resolve(join(prototypeRoot, nested));
    return contained(prototypeRoot, candidate)
      ? [candidate, join(prototypeRoot, "customer-index.html"), join(prototypeRoot, "index.html")]
      : [];
  }
  ...
  try {
    ...
    if (!filePath) {
      ...
      if (requested.startsWith("/app/customer/")) {
        filePath = join(prototypeRoot, "customer-index.html");
...
server.listen(port, "127.0.0.1", () => {
  ...
  console.log(`Full customer preview running at http://localhost:${port}/app/customer/#/dashboard`);
