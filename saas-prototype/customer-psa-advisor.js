(() => {
  ...
  function optionMarkup() {
    return state.opportunities.map(item => {
      ...
      const displayLabel = recommendation ? `${label} — ${recommendation}` : label;
      return `<option value="${escapeHtml(id)}" ${id === state.selectedId ? "selected" : ""}>${escapeHtml(displayLabel)}</option>`;
    }).join("");
  }
  ...
  function pageMarkup() {
    ...
    return `<div class="page staging-page customer-intelligence-page"><header class="page-heading"><div><span class="eyebrow">Existing PSA intelligence</span><h1>PSA Advisor</h1><p>Review saved grading context and verification requirements for one saved card.</p></div><div class="page-actions"><button class="button button-secondary" type="button" data-psa-refresh>Refresh</button></div></header>${state.loading ? `<div class="staging-loading" role="status">Loading saved PSA guidance…</div>` : ""}${errorPanel(state.error)}${configured && !state.loading && hasSavedCards ? `<section class="panel"><header class="panel-header"><div><h2>Choose a saved card</h2><p>PSA guidance is only shown for records already saved to your account.</p></div></header><div class="panel-body"><div class="field"><label for="psa-advisor-card">Saved card</label><select id="psa-advisor-card">${optionMarkup()}</select></div></div></section>${guidanceMarkup()}` : ""}${configured && !state.loading && !state.error && !hasSavedCards ? `<section class="panel"><div class="panel-body staging-empty"><strong>No saved cards yet.</strong><p>Evaluate a card first. FlipForge will not fabricate PSA guidance without a saved record.</p><a class="button button-primary" href="#/evaluate">Evaluate a card</a></div></section>` : ""}${!configured && state.health && !state.loading ? `<section class="panel"><div class="panel-body staging-empty"><strong>PSA Advisor is safely offline.</strong><p>The customer gateway is disabled, so no account request was attempted and no mock PSA data was substituted.</p></div></section>` : ""}</div>`;
  }
