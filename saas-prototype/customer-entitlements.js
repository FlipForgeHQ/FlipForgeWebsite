(() => {
  ...
  function planCards(plans, data) {
    ...
    return `<div class="customer-entitlement-plans">${safePlans.map(plan => {
      ...
      return `<article class="panel customer-entitlement-plan"><div class="panel-body"><div class="customer-entitlement-plan-head"><div><span class="eyebrow">Planned launch plan</span><h3>${escapeHtml(plan.name || plan.code || "Plan")}</h3></div>${status}</div><ul>${planFeature("Monthly evaluations", plan.monthlyEvaluationLimit)}${planFeature("Tracked cards", plan.trackedCardLimitLabel || plan.trackedCardLimit)}${planFeature("Full evidence", plan.fullEvidenceReview)}${planFeature("Decision traceback", plan.decisionTraceback)}${planFeature("PSA intelligence", plan.psaIntelligence)}${planFeature("CSV exports", plan.csvExports)}${planFeature("Batch evaluation", plan.batchEvaluation)}</ul>${checkoutEligible ? `<div class="customer-checkout-action">${checkoutButton(plan, data)}</div>` : ""}</div></article>`;
    }).join("")}</div>`;
  }
