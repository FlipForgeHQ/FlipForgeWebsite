(() => {
  ...
  function correlationId() {
    ...
    return `card-cockpit-${Date.now()}-${Math.random().toString(16).slice(2)}`;
  }
  ...
  function uniqueHeatItems(data) {
    ...
    for (const item of [...safeArray(data?.top5), ...safeArray(data?.hiddenGems), ...safeArray(data?.highestEdge)]) {
      ...
      if (!SAFE_ID.test(id) || map.has(id)) continue;
      map.set(id, item);
    }
    return map;
  }
  ...
  function loadingPanel() {
    return `<section class="forge-heat-card" data-forge-cockpit data-forge-cockpit-state="loading" aria-live="polite">
      <div class="forge-heat-card-head">
        <div class="forge-heat-rank" aria-hidden="true">🔥</div>
        <div class="forge-heat-identity"><span class="forge-heat-kicker">PRO INTELLIGENCE</span><h3>Checking Forge Heat…</h3><p>Loading the server-owned opportunity ranking for this saved decision.</p></div>
      </div>
    </section>`;
  }

  function lockedPanel(data) {
    return `<section class="forge-heat-lock" data-forge-cockpit data-forge-cockpit-state="locked">
      <span class="forge-heat-lock-mark" aria-hidden="true">PRO</span>
      <div><span class="eyebrow">Forge Heat™</span><h2>Premium opportunity intelligence</h2><p>${escapeHtml(data?.upgradeMessage || "Forge Heat is included with FlipForge Pro.")}</p><p>Smart Opportunity remains the recommendation authority; no Heat score is exposed for this plan.</p><a class="button button-primary" href="#/account">View Plan &amp; Usage</a></div>
    </section>`;
  }

  function scoredPanel(item) {
    return `<section class="forge-heat-card" data-forge-cockpit data-forge-cockpit-state="scored">
      <div class="forge-heat-card-head">
        <div class="forge-heat-rank" aria-hidden="true">🔥</div>
        <div class="forge-heat-identity"><span class="forge-heat-kicker">PRO INTELLIGENCE · SERVER-OWNED</span><h3>Forge Heat™</h3><p>This opportunity is currently surfaced by Forge Heat V1. Smart Opportunity still owns the ${escapeHtml(item.recommendation || "saved")} decision.</p></div>
        <div class="forge-heat-score" aria-label="Forge Heat ${escapeHtml(item.heat)} out of 100"><strong>${escapeHtml(item.heat)}</strong><span>${escapeHtml(bandLabel(item.heatBand))}</span></div>
      </div>
      ...
      <div class="forge-heat-card-foot"><span>Heat prioritizes the opportunity. It does not bid, buy, sell, or change BUY/WATCH/VERIFY/PASS.</span><a class="button button-secondary" href="#/forge-heat">Open Forge Heat →</a></div>
    </section>`;
  }

  function withheldPanel(unscored) {
    return `<section class="forge-heat-card" data-forge-cockpit data-forge-cockpit-state="withheld">
      <div class="forge-heat-card-head">
        <div class="forge-heat-rank" aria-hidden="true">—</div>
        <div class="forge-heat-identity"><span class="forge-heat-kicker">PRO INTELLIGENCE · EVIDENCE GATE</span><h3>Forge Heat withheld</h3><p>FlipForge will not manufacture an opportunity score when the saved decision does not clear the Heat gates.</p></div>
      </div>
      <div class="forge-heat-roadmap-boundary"><strong>Why no Heat score</strong><ul>${list(unscored?.reasons)}</ul></div>
      <div class="forge-heat-card-foot"><span>Smart Opportunity remains the decision authority.</span><a class="button button-secondary" href="#/forge-heat">Open Forge Heat →</a></div>
    </section>`;
  }

  function notSurfacedPanel() {
    return `<section class="forge-heat-card" data-forge-cockpit data-forge-cockpit-state="not-surfaced">
      <div class="forge-heat-card-head">
        <div class="forge-heat-rank" aria-hidden="true">•</div>
        <div class="forge-heat-identity"><span class="forge-heat-kicker">PRO INTELLIGENCE</span><h3>Not currently surfaced</h3><p>This saved decision is not in the current server-returned Top 5, Hidden Gems, or Highest Edge set. The browser does not infer a Heat score.</p></div>
      </div>
      <div class="forge-heat-card-foot"><span>Current V1 scope is your saved evaluated universe, not continuous market-wide scanning.</span><a class="button button-secondary" href="#/forge-heat">Review Forge Heat →</a></div>
    </section>`;
  }

  function errorPanel(error) {
    return `<section class="forge-heat-card" data-forge-cockpit data-forge-cockpit-state="error">
      <div class="forge-heat-card-head"><div class="forge-heat-rank" aria-hidden="true">!</div><div class="forge-heat-identity"><span class="forge-heat-kicker">PRO INTELLIGENCE</span><h3>Forge Heat unavailable</h3><p>${escapeHtml(error?.message || "The server-owned Heat projection could not be loaded.")}</p></div></div>
      <div class="forge-heat-card-foot"><span>No fallback or browser-generated Heat score was substituted.</span><a class="button button-secondary" href="#/forge-heat">Open Forge Heat →</a></div>
    </section>`;
  }

  function panelFor(opportunityId) {
    if (state.loading) return loadingPanel();
    if (state.error) return errorPanel(state.error);
    const data = state.payload?.data;
    if (!data) return loadingPanel();
    if (data.locked === true) return lockedPanel(data);
    const item = uniqueHeatItems(data).get(opportunityId);
    if (item) return scoredPanel(item);
    const unscored = unscoredReason(data, opportunityId);
    if (unscored) return withheldPanel(unscored);
    return notSurfacedPanel();
  }
  ...
  function mount() {
    ...
    hero.insertAdjacentHTML("beforebegin", panelFor(opportunityId));
  }
  ...
  async function ensureHeatForRoute() {
    ...
    if (state.routeId === opportunityId && (state.loading || state.payload || state.error)) {
      mount();
      return;
    }
  ...
  window.FlipForgeCustomerCardCockpit = {
    version: "1.0",
