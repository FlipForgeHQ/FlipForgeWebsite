...
[
  ...
  ["003 deploy previews replace mock Opportunity routes", files.routeHook.includes('route === "opportunities"') && files.routeHook.includes("adapter.renderCustomer(main, id)")],
  ["004 production keeps the existing prototype routes", files.routeHook.includes("evaluationAdapter.isEligible()") && files.routeHook.includes("adapter.isEligible()")],
  ...
  ["022 browser adapters contain no service token", !/FLIPFORGE_API_SERVICE_TOKEN/.test(files.readAdapter + files.evaluationAdapter)],
  ["023 browser adapters do not persist auth or request state", !/localStorage|sessionStorage|document\.cookie/.test(files.readAdapter + files.evaluationAdapter)],
