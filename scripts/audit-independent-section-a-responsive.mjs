...
for (const viewport of viewports) {
  ...
  // The local static harness does not run the production analytics function. Stub only
  // that non-product telemetry endpoint so a deliberate local 501 cannot masquerade as
  // a customer-page JavaScript failure.
  ...
  for (const route of routes) {
    ...
    if (viewport.name==='desktop') {
      // Homepage intentionally uses a separate focused header. Require at least one
      // visible navigation link from either supported desktop shell rather than forcing
      // the marketing-page `.desktop-nav` class onto the homepage.
      const desktopLinks=await page.locator('.desktop-nav a:visible, .ff-home-header a:visible, header.site-header a:visible').count();
