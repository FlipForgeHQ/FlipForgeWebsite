...
function evidenceData(id) {
  return {
    ...
    linkedEvidence: [{
      sourceName: id === "qa-a" ? "Authorized QA sold evidence A" : "Authorized QA sold evidence B",
      type: "SOLD_COMP",
...
function marketViewData() {
  ...
  return {
    ...
    summary: {
      ...
      actionableSharePct: evaluatedCards ? 100 / evaluatedCards : 0,
    ...
    decisionMix: {
      BUY: 1,
      WATCH: Math.max(0, evaluatedCards - 1),
      VERIFY: 0,
      PASS: 0,
      OTHER: 0
    },
    evidenceHealth: {
      strongEvidenceCards: evaluatedCards,
    ...
    broaderMarket: {
      ...
      reason: "Governed market-wide scanner and history inputs are not active yet."
    }
  };
}

function compareData(ids) {
  const items = ids.map(id => opportunities.find(item => item.id === id)).filter(Boolean);
  return {
    kind: "compare",
    readOnly: true,
    count: items.length,
    sameExactCardIdentity: items.length === 2 && items[0].cardIdentity === items[1].cardIdentity,
...
async function poll(predicate, message, timeoutMs = 8000) {
  ...
  while (Date.now() - started < timeoutMs) {
    try {
      ...
      // A real SPA route transition can replace the prior execution context.
    }
...
async function advancedNavLink(route) {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    try {
      ...
      await details.waitFor({ state: "attached", timeout: 4000 });
  ...
  throw lastError || new Error(`Advanced navigation link ${href} was unavailable`);
}
...
async function waitExitReady(id = "qa-a") {
  await poll(
    () => callsFor(`/api/v1/opportunities/${id}`).length > 0 && callsFor(`/api/v1/evidence/${id}`).length > 0,
    `Exit Review did not read ${id} context`
  );
...
function evaluatedCardsValue() {
  return page
    .locator("#main-content .market-view-metrics .market-view-metric")
    .filter({ hasText: "Evaluated cards" })
    .first()
    .locator("strong")
    .first();
}
...
try {
  ...
  await page.route("**/assets/js/flipforge-identity.js", route => route.fulfill({
    ...
    body: `window.FlipForgeIdentity = Object.freeze({ getUser: () => ({ email: "${email}" }), getSnapshot: () => ({ authenticated: true, email: "${email}", fullName: "Pre-beta Cross-Surface Audit", membershipActive: true, membershipConfigured: true }) });`
  }));
  ...
  await page.route("**/api/v1/**", async route => {
    ...
    if (url.pathname === "/api/v1/health") {
      return fulfill({ meta: { contractVersion: "1.0", correlationId }, data: { status: "configured" } });
    }
    if (url.pathname === "/api/v1/opportunities") {
      return fulfill(envelope(correlationId, { kind: "opportunities", items: opportunities }));
    }
    if (url.pathname === "/api/v1/lifecycle") {
      return fulfill(envelope(correlationId, { kind: "lifecycle", sourceOfTruth: "SQLite", items: lifecycleItems }));
    }
    ...
    if (lifecycleMatch) {
      ...
      return fulfill(envelope(correlationId, {
        kind: "lifecycle-detail",
        opportunityId: lifecycleMatch[1],
        lifecycle,
        history: []
      }));
    }
    ...
    if (url.pathname === "/api/v1/market-view") {
      return fulfill(envelope(correlationId, marketViewData(), badMarketAuthority ? "Browser Fake" : "Smart Opportunity"));
    }
    if (url.pathname === "/api/v1/compare") {
      const ids = String(url.searchParams.get("ids") || "").split(",").filter(Boolean);
      return fulfill(envelope(correlationId, compareData(ids)));
    }
    const opportunityMatch = url.pathname.match(/^\/api\/v1\/opportunities\/(qa-[ab])$/);
    if (opportunityMatch) {
      return fulfill(envelope(correlationId, {
        kind: "opportunity-detail",
        opportunity: opportunities.find(item => item.id === opportunityMatch[1])
      }));
    }
  ...
  await runScenario("Portfolio renders only governed reference context and no invented holding", async () => {
    await openRoute("portfolio");
    await waitPortfolioReady();
  ...
  await runScenario("Portfolio to Alerts transition cannot leave stale Portfolio ownership", async () => {
    await openRoute("portfolio");
    await waitPortfolioReady();
    await clickAdvancedRoute("alerts", waitAlertsReady);
  ...
  await runScenario("Forge Heat locked state cannot surface fabricated opportunities", async () => {
    await openRoute("forge-heat");
    await waitForgeHeatLocked();
  ...
  await runScenario("Forge Heat to Market View transition removes stale Heat UI", async () => {
    await openRoute("forge-heat");
    await waitForgeHeatLocked();
    await clickAdvancedRoute("market-view", waitMarketViewReady);
  ...
  await runScenario("Market View revisit reloads fresh server state instead of cached summary", async () => {
    ...
    await openRoute("market-view");
    await waitMarketViewReady();
    expect((await evaluatedCardsValue().innerText()).trim() === "1", "Initial Market View summary did not show one evaluated card");
    ...
    await openRoute("market-view");
    await waitMarketViewReady();
    expect((await evaluatedCardsValue().innerText()).trim() === "2", "Market View reused stale cached summary after revisit");
  });

  await runScenario("Compare keeps two saved records independent and creates no winner", async () => {
    await openRoute("compare");
  ...
  await runScenario("Compare swap cannot mix or duplicate saved identities", async () => {
    await openRoute("compare");
    await waitCompareReady();
    const before = callsFor("/api/v1/compare").length;
    await page.locator("[data-compare-swap]").click();
    await poll(() => callsFor("/api/v1/compare").length > before, "Compare swap did not issue a fresh comparison read");
    await waitCompareReady();
  ...
  await runScenario("PSA Advisor reads saved guidance and does not recalculate or predict a grade", async () => {
    await openRoute("psa-advisor/qa-a");
    await waitPsaReady("qa-a");
  ...
  await runScenario("Evidence record switch cannot leak the prior card identity", async () => {
    ...
    await poll(() => page.url().includes("#/evidence/qa-b"), "Evidence route did not move to qa-b");
    ...
    await poll(
      () => page.locator("#main-content").innerText().then(text => /Authorized QA sold evidence B/.test(text)),
      "qa-b evidence never became the visible record"
    );
    const text = await page.locator("#main-content").innerText();
    expect(!/Authorized QA sold evidence A/.test(text), "qa-a evidence leaked into qa-b after record switch");
  });

  await runScenario("Exit Review uses saved context only and exposes no sell transaction action", async () => {
    await openRoute("sell/qa-a");
    await waitExitReady("qa-a");
    const text = await page.locator("#main-content").innerText();
    ...
    const actions = (await page.locator("#main-content a, #main-content button").allTextContents()).join(" ");
    expect(!/create listing|list now|sell now|accept offer|checkout|pay now/i.test(actions), "Exit Review exposed transaction/listing authority");
  });
  ...
  await runScenario("Rapid cross-surface route churn settles on the final governed owner", async () => {
    ...
    let link = await advancedNavLink("market-view");
    await link.click();
    await poll(() => page.url().includes("#/market-view"), "Route churn did not reach Market View");

    link = await advancedNavLink("compare");
    await link.click();
    await poll(() => page.url().includes("#/compare"), "Route churn did not reach Compare");

    link = await advancedNavLink("portfolio");
    await link.click();
    await poll(() => page.url().includes("#/portfolio"), "Route churn did not reach Portfolio");
    ...
    expect(
      await page.locator("#main-content .forge-heat-shell, #main-content .market-view-shell, #main-content .customer-compare-page").count() === 0,
      "A stale prior surface repainted after final route ownership"
    );
  });
  ...
  await runScenario("Cross-surface browser requests never supply tenant identity or recommendation transaction authority", async () => {
    expect(
      calls.filter(call => call.headers["x-flipforge-tenant-id"] || call.headers["x-flipforge-user-id"] || call.headers.authorization).length === 0,
      "Browser supplied forbidden tenant, user, or authorization headers"
    );
    expect(
      calls.filter(call => Object.keys(call.body || {}).some(key => /tenantId|userId|transactionAuthority|recommendationAuthority/i.test(key))).length === 0,
      "Browser request body attempted to supply tenant or authority fields"
    );
...
if (!failures.length) {
  console.log("PASS | Alerts, Portfolio, Forge Heat, Market View, Compare, PSA Advisor, Evidence, Exit Review, route ownership, freshness, and authority boundaries are intact");
}
