...
[
  ...
  ["060 Beta Guide identifies real provider-backed Discover boundary", files.beta.includes("Start with Discover") && files.beta.includes("Provider may be unavailable") && files.beta.includes("Discovery score") && files.beta.includes("not BUY/WATCH/VERIFY/PASS")],
  ["061 Beta Guide links completed lifecycle lane", files.beta.includes("Tracking → Portfolio → Alerts") && files.beta.includes("tenant-scoped SQLite lifecycle records")],
  ["062 Beta Guide discloses remaining lifecycle limits", files.beta.includes("Current value and performance remain unavailable") && files.beta.includes("External alert delivery is not connected")],
  ["063 private Beta docs identify real management routes", files.betaDocs.includes("Dashboard, Discover, Evaluate, Opportunities") && files.betaDocs.includes("Evidence Center, saved PSA guidance, Exit Review, Tracking, Portfolio, Alerts")],
  ["064 private Beta docs identify authoritative lifecycle routes", files.betaDocs.includes("Tracking persists watch status") && files.betaDocs.includes("Alerts project persisted in-app review rules")],
