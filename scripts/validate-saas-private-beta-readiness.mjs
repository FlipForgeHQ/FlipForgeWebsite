...
[
  ...
  ["039e onboarding keeps feedback after the first-decision journey", beta.indexOf('class="private-beta-v2-feedback"') > beta.indexOf('class="private-beta-v2-walkthrough"')],
  ...
  ["043 feedback posts JSON to the same-origin signed endpoint", beta.includes("fetch(FEEDBACK_ENDPOINT") && beta.includes('"Content-Type": "application/json"')],
  ["044 feedback posts through same-origin credentials", beta.includes('credentials: "same-origin"')],
  ["045 feedback payload names are allowlisted", beta.includes('["category", "rating", "summary", "expected", "contactAllowed", "checkpoint", "outcome"]')],
  ["046 feedback payload includes route context only", beta.includes("payload.route = routeName()")],
  ["047 feedback email is never supplied by the browser", !beta.includes("testerEmail") && !beta.includes("payload.email") && !beta.includes("session.email")],
