...
const proof = {
  ...
  evidence: { acceptedSales: 3, averagePrice: 500, latestSaleDate: "2026-09-13" }
};
...
try {
  for (const viewport of viewports) {
    ...
    for (const route of routes) {
      ...
      } else if (state.root.left < 10 || state.root.right > state.innerWidth - 10) {
        fail(`mobile/${route}: route content escaped the governed mobile gutters`, state);
      }

      if (state.heading && baseline.heading && !closeEnough(state.heading.fontSize, baseline.heading.fontSize, 0.25)) {
        fail(`${viewport.name}/${route}: customer page title size drifted`, { baseline: baseline.heading, state: state.heading });
      }
      if (state.heading) {
        const [min, max] = viewport.name === "mobile" ? [27, 33] : [31.5, 42];
        if (state.heading.fontSize < min || state.heading.fontSize > max) {
          fail(`${viewport.name}/${route}: customer page title escaped the #384 visual scale`, state.heading);
        }
      }
    }
