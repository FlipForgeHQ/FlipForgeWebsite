...
[
  ...
  ["044 identity selection is bound to the exact result query and request generation", discovery.includes("const selectionOwnerQuery = normalizeIdentityQuery(state.identityAssist.resultsQuery)") && discovery.includes("const selectionOwnerSerial = state.identityAssist.resultsSerial") && discovery.includes("resolveIdentity(index, selectionOwnerQuery, selectionOwnerSerial)")],
  ["045 stale selection fails closed against result generation, assist query, draft and visible query", discovery.includes("expectedSerial !== currentSerial") && discovery.includes("expectedSerial !== resultSerial") && discovery.includes("ownedQuery !== currentQuery") && discovery.includes("ownedQuery !== resultQuery") && discovery.includes("ownedQuery !== draftQuery") && discovery.includes("ownedQuery !== visibleQuery")],
